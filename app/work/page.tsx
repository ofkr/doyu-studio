...
type Category = 'All' | 'Graphic Design' | 'Wedding Snap' | 'Wedding Film'
type GraphicSubTab = '전체' | '커스텀 가능' | '커스텀 불가'
...
const normWorkCat = (cat: string | null | undefined) => (cat ? WORK_EN[cat] ?? cat : '')

const categories: Category[] = ['All', 'Graphic Design', 'Wedding Snap', 'Wedding Film']
const graphicSubTabs: GraphicSubTab[] = ['전체', '커스텀 가능', '커스텀 불가']
...
export default function WorkPage() {
  ...
  const [activeSubTab, setActiveSubTab] = useState<GraphicSubTab>('전체')
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const { data, error } = await supabase.from('works').select('*').eq('is_published', true).order('id')
  ...
  const filteredItems = workItems.filter((item) => {
    ...
    const ko = WORK_KO[activeCategory]
    const cat = item.category ?? ''
    const matches = cat === activeCategory || cat === ko
    if (!matches) return false
    if (activeCategory === 'Graphic Design') {
      const canCustom = item.custom_available ?? false
      if (activeSubTab === '커스텀 가능') return canCustom
      if (activeSubTab === '커스텀 불가') return !canCustom
    }
    return true
  })
      ...
      <div className="flex items-start">
        ...
        <main className="flex flex-col flex-1 px-5 pt-24 w-full">
          <section className="pt-[60px] md:pt-[80px] pb-[40px]">
            <h1 className="text-[48px] md:text-[100px] font-black leading-none text-[#2d4a1e]">WORK</h1>
            <p className="text-sm text-gray-500 mt-3">DOYU STUDIO의 작업물을 소개합니다.</p>
          </section>
          ...
          <section className="py-8">
            {loading ? (
              ...
              <p className="text-sm text-gray-400 py-20 text-center">작업물이 없습니다.</p>
            ) : (
                  ...
                  >
                    <div className="px-3 pt-3 pb-1">
                      <span className="text-[9px] font-medium tracking-widest uppercase text-gray-400">
                        {normWorkCat(item.category)}
                      </span>
                    </div>
                    <div className="relative w-full px-3">
                      {(() => {
                        const thumb = item.video_url ? getYoutubeThumbnail(item.video_url) : null
                        const src = thumb ?? item.image ?? null
                        return src ? (
                          <Image
                            src={src}
                            alt={item.title ?? ''}
                    ...
                    <div className="px-3 pt-2 pb-4">
                      ...
                      <div className="flex justify-between items-center mt-1.5">
                        {normWorkCat(item.category) === 'Wedding Snap' ? (
                          <span className="text-[10px] text-gray-400">스냅 촬영</span>
                        ) : normWorkCat(item.category) === 'Wedding Film' ? (
                          <span className="text-[10px] text-gray-400">영상 촬영</span>
                        ) : (
                          <span className={`text-[10px] ${(item.custom_available ?? false) ? 'text-[#2d4a1e]' : 'text-gray-400'}`}>
                            {(item.custom_available ?? false) ? '커스텀 가능' : '커스텀 불가'}
                          </span>
                        )}
                      </div>
                    </div>
